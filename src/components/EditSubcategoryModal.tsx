      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white font-display flex items-center gap-2">
            ...
            {isEditing ? '✏️ Rename Subcategory' : '🏷️ New Subcategory'}
          </h3>
          <button 
            id="close-subcategory-modal-btn"
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label htmlFor="subcat-name" className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">
              Subcategory Name
            </label>
            <input
              id="subcat-name"
              type="text"
              required
              placeholder="e.g., Spine, Knee, Thorax"
              value={name}
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end space-x-3">
            ...
            >
              {isEditing ? 'Apply Rename' : 'Add Subcategory'}
            </button>
          </div>
        </form>
      </div>
